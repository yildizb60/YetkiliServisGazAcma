...
|---|---|---|
| 1 | Giriş yerleşimi | Açıklamalı geniş sol panel kaldırıldı. Dar manzara alanı, üst logo ve sade giriş formu var. Eski kombili görsel kullanılmıyor; mavi/yeşil renkler korundu. |
| 2 | Yeni talep | Çok adımlı yoğun görünüm yerine tesisat sorgusu; başarılı sorgudan sonra cihaz seçimi ve dört yeni cihaz alanı. Teknik açıklama paragrafları kaldırıldı. |
...
| 4 | Detay kullanılabilirliği | Rol ayrımı var. Güncel kontrol/imza işlemi ikincil bilgilerden önce; karşılaştırma, kayıt bilgileri ve eski geçmiş açılır alanlarda. Tamamlanan kayıtta yinelenen imza alanı kapalı başlar. |
| 5 | Form adı | Görünen önizleme komutu “Formu Önizle”. Şirket başına gerçek belge şablonu seçimi henüz yok; mevcut resmi şablon korunuyor. |
...
| 8 | Menü grupları | Ortak sidebar'da cihaz değişimi ile yetkili servis/devreye alma işleri ayrıldı. Firma yalnızca kendi menülerini görür. |
| 9 | Yetki belgesi depolama | Dosya App_Data altında şirket/firma/yıl grubunda; DB'de metadata ve depolama anahtarı. İndeks SQL scripti var, bu tur DB'de çalıştırılmadı. |
...
| 21 | Ana panel takvimi | İç rollerde bugünkü randevular, ayrıca tarih/il/bölge/personel filtreli ajanda ve sunucu sayfalaması eklendi. Firma dashboard'ı değiştirilmedi. |
...
- Hazır indeksler: `DatabaseScripts/2026-09-04_yetki_belgesi_depolama_indeksleri.sql` ve `DatabaseScripts/2026-09-07_ykc_randevu_takvim_indeksi.sql`. Migration eklenmedi, SQL çalıştırılmadı.
- Bağımsız kural kontrolleri: `dotnet run --project Tests/YkcRules/YkcRules.csproj --no-launch-profile`. Snapshot kapsamı, eski veri manipülasyonu, baştaki sıfırların normalleştirilmesi, kapasite ve dakika sınırları dahil 23 kontrol; veritabanına dokunmaz.
...
## Bu Turdaki Ekran Kontrolleri

- Sertifikalı firma rolünde giriş, tesisat sorgusu, cihaz seçimi, talep listesi, mevcut talep detayı ve resmi form önizlemesi gerçek tarayıcıda açıldı. Yeni talep veya randevu kaydı oluşturulmadı.
- Firma ekranlarında eski cihaz marka/kapasitesi gizli, resmi formda görünür. Liste son işlem sütunu masaüstünde görünür; mobil görünümde yatay sayfa taşması görülmedi.
- Personel oturumuna geçiş tarayıcı güvenlik onayına takıldı; bu tur personel ekranları için yeni canlı test tamamlandı denilmiyor.
...
Giriş için kullanılan yeni görsel: `YetkiliServisGazAcma/wwwroot/images/hero/login-landscape.png`. Üretim yönlendirmesi: fotogerçekçi İç Anadolu yeşil kırları, alçak dağlar ve geniş mavi gökyüzü; bina, kişi, kombi, metin ve logo yok. Aksa referansı yalnızca yerleşim içindir; marka unsurları kopyalanmadı.
